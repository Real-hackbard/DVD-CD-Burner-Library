...
type TScsiModePageTemplate10 = packed record
...
type TScsiModePageTemplate2 = packed record
         ModeDataLength10   : WORD;  // full record length except itself
         MediumType10       : BYTE;  // Reserved - OBSOLETE
         DeviceSpecific10   : BYTE;  // Reserved - OBSOLETE
         Reserved10         : WORD;
...
type TScsiModePageTemplate6 = packed record
...
type TScsiCDBufferInfo = packed record
      DataLength   :  Word;
      Reserved1    :  BYTE;
      Reserved2    :  BYTE;
      SizeOfBuffer :  DWORD;
...
type TScsiDeviceConfigHeader = Packed Record   // from ntddmmc.h - MMC 3 Feature structures
     DataLength        : Cardinal;
     Reserved          : Word;
  ...
  TScsiDVDLayerDescriptorInfo = record
    ...
    LayerType,
    NoLayer : String;
    Sectors : Integer; // total sectors - result of  (EndPhysicalSector minus StartingPhysicalSector)
  end;
...
type TScsiWriteModePage = packed Record
        PSPageCode            : byte;  {PS bit 7} {PageCode bits 0-6}
        PageLength            : byte;  {page length = $32}
        TestFlagWriteType     : byte;  {Write type bits 0-3} {TestFlag bit 4} {link size bit 5} {buffer free bit 6}
        MSFPCopyTrackMode     : byte;  {MS (Multisession) bits 6-7} {FP bit 5} {Copy bit 4} {Track mode bits 0-3}
        DataBlockType         : byte;  {datablock type bits 0-3}
        LinkSize              : Byte;  {link size = 7}
        Reserved1             : Byte;
        ...
        InternationalStandardRecordingCode : array [1..16] of byte;
