...
uses windows,SCSITypes, scsidefs,CovertFuncs,Sysutils,{ASPIUnit,}SptiUnit,wnaspi32;
...
function SCSIReadBufferCapacity(DeviceID : TCDBurnerInfo;
                 Buf : pointer;  var Sdf : TScsiDefaults) : TScsiError;
Function SCSICloseSession(DeviceID :TCDBurnerInfo;var Sdf : TScsiDefaults) : TScsiError;

Function SCSICloseTrack(DeviceID :TCDBurnerInfo;Track:byte;
var Sdf : TScsiDefaults) : TScsiError;

Function SCSISYNCCACHE(DeviceID :TCDBurnerInfo; var Sdf : TScsiDefaults) : TScsiError;
...
Function ScsiGetWriteParams(DevID :TCDBurnerInfo;Size: Integer; Var Param : String;
                     var Sdf : TScsiDefaults) : TScsiError;
...
function SCSIgetTOC(DeviceID : TCDBurnerInfo; var TOC : TScsiTOC; var Sdf : TScsiDefaults) : TScsiError;


function SCSIReadTrackInformation(DeviceID : TCDBurnerInfo; Const ATrack : Byte; Var TrackInformation : TTrackInformation; var Sdf : TScsiDefaults) : TScsiError;
function SCSIReadFormatCapacity(DeviceID : TCDBurnerInfo; var FormatCapacity : TFormatCapacity; var Sdf : TScsiDefaults) : TScsiError;
function SCSIReadDiscInformation(DeviceID : TCDBurnerInfo; var DiscInformation : TDiscInformation; var Sdf : TScsiDefaults) : TScsiError;
function SCSIReadDVDStructure(DeviceID : TCDBurnerInfo;  Var DescriptorStr : TScsiDVDLayerDescriptorInfo; var Sdf : TScsiDefaults): TScsiError;
function SCSIGetDevConfigProfileMedia(DeviceID : TCDBurnerInfo; var ProfileDevDiscType : TScsiProfileDeviceDiscTypes; var Sdf : TScsiDefaults) : TScsiError;
...
var Arg1 : byte;
    ...
    Lba : DWORD;
begin
   if ASPIgetDeviceIDflag(DeviceID.DriveID, ADIDmodeSenseDBD)
...
begin
   ...
   if ASPIgetDeviceIDflag(DeviceID.DriveID, ADIDmodeSense6) then begin
      ...
      dblen := Integer(BigEndianW(Src10.DescriptorLength10));        // 2nd break point - dblen=integer & BigIndian returns DWORD
   ...
   if mp > Integer(ps^.ParamLength)  then mp := Integer(ps^.ParamLength);     // 3rd break point
   if BufLen > (DWORD(mp) - BufPos) then BufLen := (DWORD(mp) - BufPos);
...
function SCSImodeSelectPage(DeviceID : TCDBurnerInfo; PageCode : BYTE;
                ...
                var Sdf : TScsiDefaults) : TScsiError;
var
    Chg10 ,Src10 : TScsiModePageTemplate2;
    SdfTemp    : TCdRomModePageType;
    pp, ps, gs : PScsiModePageRecord;
    i, mp, len, dblen : integer;
begin
...
begin
  ...
  Value.CurrentWriteSpeed := Round(SwapWord(ModePage.CurWriteSpeed_Res)  / 176.46);
...
begin
   ...
   if (FB and CDSTATUS_AUDIO_COMPOSITE ) <> 0 then Include(Value, cdcAudioComposite);
   if (FB and CDSTATUS_AUDIO_DIGIPORT1 ) <> 0 then Include(Value, cdcAudioDigiPort1);
   if (FB and CDSTATUS_AUDIO_DIGIPORT2 ) <> 0 then Include(Value, cdcAudioDigiPort2);
   if (FB and CDSTATUS_READ_MODE2_FORM1) <> 0 then Include(Value, cdcReadMode2form1);
   if (FB and CDSTATUS_READ_MODE2_FORM2) <> 0 then Include(Value, cdcReadMode2form2);
   if (FB and CDSTATUS_READ_MULTISESSION)<> 0 then Include(Value, cdcReadMultisession);
   if (FB and CDSTATUS_BURN_PROOF ) <> 0 then Include(Value, cdcWriteBurnProof);
...
procedure InquiryDecodePeripherals(Arg : BYTE; var PQ : TScsiPeripheralQualifier; var DeviceType : TScsiDeviceType);
begin
...
function SCSIseek10(DeviceID : TCDBurnerInfo;GLBA : DWORD; var Sdf : TScsiDefaults) : TScsiError;
var
   CDB : TCDB10;
begin
   FillChar(cdb,10,0);
...
begin
   ...
   TOC.TrackCount := (BigEndianW(Buf.Length) div SizeOf(TScsiTrackDescriptorTemplate));// -1; // -1 adjusted by didier schvartz
...
begin
   ...
   Result := ASPIsend12CDB(DeviceID,CDB,@Buf,SizeOf(Buf),SRB_DIR_IN,Sdf);
   Sdf.Timeout := SdfTemp;
end;
...
begin
   ...
   if Result <> Err_None then exit;
          ...
          if Test_Write = True then Buf.TestFlagWriteType := BitOn(Buf.TestFlagWriteType,4);
          if Burn_Proof = True then Buf.TestFlagWriteType := BitOn(Buf.TestFlagWriteType,6);
...
begin
    ...
    Result := ASPIsend10CDB(DeviceID,CDB,Buf,SizeOf(TScsiCDBufferInfo),SRB_DIR_IN,Sdf);
end;
...
Begin
  //cdr CDB:  BB 00 FF FF 08 4C 00 00 00 00 00 00
   ...
   cdb[2] := (ReadSpeed shr 8)  and $FF;
   cdb[3] := ReadSpeed and $ff;
   cdb[4] := (WriteSpeed shr 8) and $FF;
   cdb[5] := WriteSpeed and $ff;
...
Begin
   ...
   Case (ModePage.SessionFormat ) of
    ...
    End; //Case
    Param := Param + 'Audio Pause Length :  '+inttostr(SwapWord(ModePage.AudioPauseLength))+ #10#13;
...
begin
    ...
    If Result = Err_SenseIllegalRequest then Exit;
  ...
  Case (( DeviceConfigHeader.CurrentProfile shl 8) and $FF00) or ((DeviceConfigHeader.CurrentProfile shr 8) and $00FF) Of
   $0000 : begin ProfileDevDiscType.SubType := 'pdtNoCurrentProfile'; ProfileDevDiscType.DType := 'NONE';ProfileDevDiscType.TypeNum := 0; end;
   $0001 : begin ProfileDevDiscType.SubType := 'pdtNonRemovableDisk'; ProfileDevDiscType.DType := 'NonRemovable';ProfileDevDiscType.TypeNum := 1; end;
   $0002 : begin ProfileDevDiscType.SubType := 'pdtRemovableDisk'; ProfileDevDiscType.DType := 'Removable';ProfileDevDiscType.TypeNum := 2; end;
   $0003 : begin ProfileDevDiscType.SubType := 'pdtMagnetoOptical_Erasable'; ProfileDevDiscType.DType := 'Erasable';ProfileDevDiscType.TypeNum := 3; end;
   $0004 : begin ProfileDevDiscType.SubType := 'pdtOptical_WriteOnce'; ProfileDevDiscType.DType := 'WriteOnce';ProfileDevDiscType.TypeNum := 4; end;
   $0005 : begin ProfileDevDiscType.SubType := 'pdfAS-MO'; ProfileDevDiscType.DType := 'AS-MO';ProfileDevDiscType.TypeNum := 5; end;
   $0008 : begin ProfileDevDiscType.SubType := 'pdfCD-ROM'; ProfileDevDiscType.DType := 'CD';ProfileDevDiscType.TypeNum := 6; end;
   $0009 : begin ProfileDevDiscType.SubType := 'pdfCD-R'; ProfileDevDiscType.DType := 'CD';ProfileDevDiscType.TypeNum := 7; end;
   $000A : begin ProfileDevDiscType.SubType := 'pdfCD-RW'; ProfileDevDiscType.DType := 'CD';ProfileDevDiscType.TypeNum := 8; end;
   $0010 : begin ProfileDevDiscType.SubType := 'pdfDVD-ROM'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 9; end;
   $0011 : begin ProfileDevDiscType.SubType := 'pdfDVD-R'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 10; end;
   $0012 : begin ProfileDevDiscType.SubType := 'pdfDVD-RAM'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 11; end;
   $0013 : begin ProfileDevDiscType.SubType := 'pdfDVD-RW Restricted'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 13; end;
   $0014 : begin ProfileDevDiscType.SubType := 'pdfDVD-RW Sequential'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 14; end;
   $001A : begin ProfileDevDiscType.SubType := 'pdfDVD+RW'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 15; end;
   $001B : begin ProfileDevDiscType.SubType := 'pdfDVD+R'; ProfileDevDiscType.DType := 'DVD';ProfileDevDiscType.TypeNum := 16; end;
   $0020 : begin ProfileDevDiscType.SubType := 'pdfDDCD-ROM'; ProfileDevDiscType.DType := 'DDcD';ProfileDevDiscType.TypeNum := 17; end;
   $0021 : begin ProfileDevDiscType.SubType := 'pdfDDCD-R'; ProfileDevDiscType.DType := 'DDcD';ProfileDevDiscType.TypeNum := 18; end;
   $0022 : begin ProfileDevDiscType.SubType := 'pdfDDCD-RW'; ProfileDevDiscType.DType := 'DDcD';ProfileDevDiscType.TypeNum := 19; end;
   $FFFF : begin ProfileDevDiscType.SubType := 'pdfUNKNOWN'; ProfileDevDiscType.DType := 'UNKNOWN';ProfileDevDiscType.TypeNum := 20; end;
  end;
end;
...
begin
    ...
    Value:=(DVDLayerDescriptor.BookType_PartVersion shr 4) and $0F;
      case Value of
       ...
       else DescriptorStr.BookType:='Unknown';
      end;
    ...
    Value:=(DVDLayerDescriptor.DiscSize_MaximumRate shr 4) and $0F;
      case Value of
       ...
       else DescriptorStr.DiscSize:='Unknown';
      end;
    ...
    Value:=(DVDLayerDescriptor.DiscSize_MaximumRate and $0F);
      case Value of
       ...
       else DescriptorStr.MaximumRate:='Unknown';
      end;
    ...
    Value:=(DVDLayerDescriptor.LinearDensity_TrackDensity shr 4) and $0F;
      case Value of
       ...
       else DescriptorStr.LinearDensity:='Reserved';
      end;
    ...
    Value:=(DVDLayerDescriptor.LinearDensity_TrackDensity and $0F);
      case Value of
       ...
       else DescriptorStr.TrackDensity:='Reserved';
      end;

    DescriptorStr.NoLayer:=IntToStr((DVDLayerDescriptor.NumberOfLayers_TrackPath_LayerType shr 5) and $03);
    ...
    Value:=(DVDLayerDescriptor.NumberOfLayers_TrackPath_LayerType and $0F);
      case Value of
       ...
       else DescriptorStr.LayerType:='Unknown';
      end;
...
begin
    ...
    SdfTemp     := Sdf.Timeout;
    Sdf.Timeout := Sdf.ReadTimeout;
    Result :=  ASPIsendScsiCommand(DeviceID, @cdb, CdbLen, Buf, BufLen, SRB_DIR_IN, Sdf);
    Sdf.Timeout := SdfTemp;
    DiscInformation.DiscInformationLength :=
      (( DiscInformation.DiscInformationLength shl 8) and $FF00) or (( DiscInformation.DiscInformationLength shr 8) and $00FF);
end;
...
begin
    ...
    begin
      Datalength          := (( Datalength shl 8) and $FF00) or (( Datalength shr 8) and $00FF); //SwapWord(Datalength);
