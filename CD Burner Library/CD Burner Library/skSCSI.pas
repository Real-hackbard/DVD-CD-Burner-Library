...
const
  ...
  HASTAT_COMMAND_TIMEOUT      = $0B; // While processing the SRB, the adapter is timed out
...
type
  ...
  public
    ...
    property viASPI: TVersionInfo read FviASPI; // Version Info of ASPI or ASAPI DLL (NIL for SPTI)
  end;
...
begin
  Path := '\\.\' + Char(i + 65) + ':'#0;
  Result := CreateFile(@Path[1], GENERIC_WRITE or GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
  if Result = INVALID_HANDLE_VALUE then
    Result := CreateFile(@Path[1], GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, 0, 0);
end;

function SPTI_SendASPI32Command_Internal(PSRB: PSRB_ExecSCSICmd; Again: Boolean): DWord;
var
  Status                      : Boolean;
...
begin
  ...
  if (SCSIDrives.Drive[Index].DeviceHandle = INVALID_HANDLE_VALUE) then
    SCSIDrives.Drive[Index].DeviceHandle := SPTI_GetDriveHandle(SCSIDrives.Drive[Index].Letter);
  FillChar(SPTI_WBUFFER, SizeOf(SPTI_WBUFFER), 0);
  if PSRB^.Flags and SRB_DIR_IN <> 0 then
    SPTI_WBUFFER.SPT.DataIn := SCSI_IOCTL_DATA_IN
  else
    if PSRB^.Flags and SRB_DIR_OUT <> 0 then
  ...
  Status := DeviceIoControl(SCSIDrives.Drive[Index].DeviceHandle, IOCTL_SCSI_PASS_THROUGH_DIRECT, @SPTI_WBUFFER, Length, @SPTI_WBUFFER, Length, Returned, nil);
  if ((SPTI_WBUFFER.SPT.SenseInfoLength = 0) or (SPTI_WBUFFER.SPT.SenseInfoLength = 32))
    and (SPTI_WBUFFER.SPT.SCSIStatus = 0) and Status then
    PSRB^.Status := SS_COMP
  else
  begin
    PSRB^.Status := SS_ERR;
    Move(SPTI_WBUFFER.ucSenseBuf, PSRB^.SenseArea, SPTI_WBUFFER.SPT.SenseInfoLength);
...
begin
  ...
  Status := DeviceIoControl(Handle, IOCTL_SCSI_PASS_THROUGH_DIRECT, P_SCSI_WBUFFER, Length, P_SCSI_WBUFFER, Length, Returned, nil);
  if not Status then
  begin
    CloseHandle(Handle);
    Exit;
  ...
  if DeviceIoControl(Handle, IOCTL_SCSI_GET_ADDRESS, nil, 0, P_SCSI_ADDRESS, SizeOf(SCSI_ADDRESS), Returned, nil) then
  begin
    pDrive.Used := True;
  ...
  case OS.dwPlatformId of
    ...
    VER_PLATFORM_WIN32_NT:
      case OS.dwMajorVersion of
        3 or 4: osName := 'Windows NT ' + IntToHex(OS.dwMajorVersion, 8) + SP + Ver;
...
begin
  ...
  { for i := 0 to CDBLen - 1 do
      CDBByte[i] := CDB[i];}
    SRB_ExecSCSICmd.CDBByte[1] := ((Lun and 7) shl 5) or (SRB_ExecSCSICmd.CDBByte[1] and $1F);
...
begin
  ...
  begin
    ...
    begin
      @GetASPI32SupportInfo := GetProcAddress(SCSIHandle, 'GetASPI32SupportInfo');
    ...
    begin
      ...
      if SCSIHandle <> 0 then
      begin
        @GetASPI32SupportInfo := GetProcAddress(SCSIHandle, 'GetASAPI32SupportInfo');
