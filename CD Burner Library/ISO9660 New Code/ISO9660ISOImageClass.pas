...
uses
   ...
   ISO9660ClassTypes,
   ISO9660BootClass,
   ISO9660PrimaryVolClass,
   ISO9660SupplementVolClass,
   ISO9660streamHandler,
   ISO9660ImageTree;
...
type
   ...
   protected
      ...
      procedure WriteStructureTree(Primary: Boolean; ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
      procedure WriteRootStructureTree(Primary: Boolean; ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
      procedure WriteFileData(ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
      procedure WritePathTableData(ISOStream: TImageStreamHandler; CurrentPointer: Integer);
      procedure WriteJolietPathTableData(ISOStream: TImageStreamHandler; CurrentPointer: Integer);
   public
   ...
   published
      ...
      property BootRecordVolumeDescriptor: TBootRecordVolumeDescriptor read FBRClass write FBRClass;
      property PrimaryVolumeDescriptor: TPrimaryVolumeDescriptor read FPVDClass write FPVDClass;
      property SupplementaryVolumeDescriptor: TSupplementaryVolumeDescriptor read FSVDClass write FSVDClass;
   end;
...
begin
   ...
   if (Assigned(FPVDClass)) then FreeAndNil(FPVDClass);
   if (Assigned(FSVDClass)) then FreeAndNil(FSVDClass);
   if (Assigned(FBRClass)) then FreeAndNil(FBRClass);
   inherited;
end;
...
begin
   FVolID := VolName;
   if (Assigned(fPVDClass)) then fPVDClass.VolumeIdentifier := VolName;
   if (Assigned(fSVDClass)) then fSVDClass.VolumeIdentifier := VolName;
end;
...
begin
   if (Assigned(fLog)) then fLog.Add(AFunction + '(): ' + AMessage);
end;
...
begin
   ...
   begin
      ...
      if (fImage.ImageFormat = ifCompleteSectors) then Log('OpenImage', 'image contains RAW data')
      else if (fImage.ImageFormat = ifOnlyData) then Log('OpenImage', 'image contains sector data');

      if (fImage.ISOBookFormat = ybfMode1) then Log('OpenImage', 'image contains yellow book mode 1 data')
      else if (fImage.ISOBookFormat = ybfMode2) then Log('OpenImage', 'image contains yellow book mode 2 data');
...
begin
   ...
   if (Assigned(fBRClass)) then fBRClass.Free;
   FBRClass := TBootRecordVolumeDescriptor.Create;

   Log('CreateImage', 'Primary Volume Descriptor Created'); // Primary Volume Descriptor
   if (Assigned(fPVDClass)) then fPVDClass.Free;
   FPVDClass := TPrimaryVolumeDescriptor.Create;

   Log('CreateImage', 'Supplementary Volume Descriptor Created'); // Supplementary Volume Descriptor
   if (Assigned(FSVDClass)) then FSVDClass.Free;
   FSVDClass := TSupplementaryVolumeDescriptor.Create;

   Log('CreateImage', 'Volume Descriptor Set Terminator Created'); // Volume Descriptor Set Terminator
...
procedure TISOImage.WriteRootStructureTree(Primary: Boolean; ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
var
 ...
 // done with "." ".."
   ...
   begin
      ...
      begin
         ...
         CopyMemory(@WideArray[1],@TempPWideChr[0],(Dir.ISOData.LengthOfFileIdentifier)-1);//makes it big endian wide char
         ISOStream.Stream.Write(WideArray, Dir.ISOData.LengthOfFileIdentifier);
      end;

      DIRRecSize := sizeof(TDirectoryrecord) + Dir.ISOData.LengthOfFileIdentifier; // get padding size
      if (DIRRecSize mod 2) > 0 then FImage.Stream.Write(PadByte, 1);
   end;
   ...
   begin
      ...
      begin
         ...
         FillChar(WideArray,128,0);
         CopyMemory(@WideArray[1],@TempPWideChr[0],(Fil.ISOData.LengthOfFileIdentifier)-1);//makes it big endian wide char
         ISOStream.Stream.Write(WideArray, Fil.ISOData.LengthOfFileIdentifier);
      end;

      DIRRecSize := sizeof(TDirectoryrecord) + Fil.ISOData.LengthOfFileIdentifier; // get padding size
      if (DIRRecSize mod 2) > 0 then FImage.Stream.Write(PadByte, 1);
   end;
   ...
   Log('Write Root Structure', 'Current Pos After Structure: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
...
procedure TISOImage.WriteStructureTree(Primary: Boolean; ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
var
   DirIndex, FileIndex, Padd: Integer;
   Dir: TDirectoryEntry;
...
begin
 ...
 // done with "." ".."
   ...
   Sector := (FImage.Stream.Position div FImage.SectorDataSize);
   Log('Write Structure', 'Current Pos After Structure: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
...
procedure TISOImage.WriteFileData(ISOStream: TImageStreamHandler; ADirEntry: TDirectoryEntry);
var
   DirIndex, FileIndex, Padd: Integer;
   Dir: TDirectoryEntry;
   Fil: TFileEntry;
...
procedure TISOImage.WritePathTableData(ISOStream: TImageStreamHandler; CurrentPointer: Integer);
var
   Index: Integer;
...
begin
   ...
   begin
      ...
      FImage.Stream.Write(PathRec^.LengthOfDirectoryIdentifier, 1); //ISO Path Table L
   ...
   for Index := 1 to PadBytes do
      FImage.Stream.Write(TempPadChar, 1);
   Sector := (FImage.Stream.Position div FImage.SectorDataSize);
   Log('SaveImage', 'Current Pos After LPath Tables: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
   ...
   for Index := 0 to FTree.PathTableCount - 1 do
    begin
      ...
      ReverseByte := SwapWord(PathRec^.ParentDirectoryNumber); // reverse to big endian
...
procedure TISOImage.WriteJolietPathTableData(ISOStream: TImageStreamHandler; CurrentPointer: Integer);
var
   Index: Integer;
   TempPchar, TempPadChar: Pchar;
   PathRec: PPathTableRecord;
...
begin
   ...
   begin
      PathRec := FTree.LittleEndianPathTable.Items[Index];
      Log('SaveImage', 'Write Dir Joliet LPath Name: ' + PathRec^.DirectoryIdentifierM);
      FImage.Stream.Write(PathRec^.LengthOfDirectoryIdentifierM, 1); //ISO Path Table L
      FImage.Stream.Write(PathRec^.ExtendedAttributeRecordLength, 1);
      PathRec^.JolietLocationOfExtent := (PathRec^.LocationOfExtent + SectorOffsett);
   ...
   begin
      PathRec := FTree.LittleEndianPathTable.Items[Index];
      Log('SaveImage', 'Write Dir Joliet MPath Name: ' + PathRec^.DirectoryIdentifierM[0]);
      FImage.Stream.Write(PathRec^.LengthOfDirectoryIdentifierM, 1); //ISO Path Table L
      FImage.Stream.Write(PathRec^.ExtendedAttributeRecordLength, 1);
      PathRec^.JolietLocationOfExtentM := SwapDWord(PathRec^.JolietLocationOfExtent);
...
begin
   ...
   case Mode of
       1: FImage := TImageStreamHandler.Create(fFileName, ybfMode1, ifCompleteSectors);
       2: FImage := TImageStreamHandler.Create(fFileName, ybfMode2, ifCompleteSectors);
   end;


      if (FImage.ISOBookFormat = ybfMode1) then Log('SaveImage', 'Yellow book mode 1')
      else if (FImage.ISOBookFormat = ybfMode2) then Log('SaveImage', 'Yellow book mode 2');

      Log('SaveImage', 'User data sector size is ' + IntToStr(FImage.SectorDataSize) + ' bytes');
      Log('SaveImage', 'Image data offset in image file is ' + IntToStr(FImage.ImageOffset) + ' bytes');

      if (FImage.SectorDataSize <> 2048) then
      begin
         Log('SaveImage', 'sorry, but sector size other than 2048 bytes are not yet supported...');
         Exit;
      end;
    ...
    // Start writing image data to file
     ...
     //setup primary volume descriptor
      ...
      copymemory(@FPVDClass.Descriptor.Primary.RootDirectory, @FTree.RootDirectory.RootISOData, sizeof(TRootDirectoryRecord));
      FImage.Stream.Write(FPVDClass.Descriptor, sizeof(FPVDClass.Descriptor)); //ISO Primary Volume Descriptor
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Write ISO Primary Volume Descriptor : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');

     //setup secondary volume descriptor
      FSVDClass.VolumeSpaceSize := BuildBothEndianDWord(FTree.FileLBA + FTree.FileBlocks);
      FSVDClass.PathTableSize := BuildBothEndianDWord(FTree.JolietPathTableLength);
      copymemory(@FSVDClass.Descriptor.Supplementary.RootDirectory, @FTree.RootDirectory.RootISOData, sizeof(TRootDirectoryRecord));
      FSVDClass.ResetRootExtent(FTree.JolietOffsett + 1); // make sure it starts in the right place
      FImage.Stream.Write(FSVDClass.Descriptor, sizeof(FSVDClass.Descriptor)); //ISO Secondary Volume Descriptor
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Write ISO Secondary Volume Descriptor :' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
    ...
    // write volume set terminator
      FImage.Stream.Write(FVDSTClass, sizeof(FVDSTClass)); //ISO Volume Set Terminator
      Log('SaveImage', 'Write Volume Set Terminator');

      CurrentStreamPointer := FImage.Stream.Position;
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Current Pos After Volume Descriptors: ' + inttostr(CurrentStreamPointer) + ' : ' + inttohex(CurrentStreamPointer, 6) + ' : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
     ...
     //write out Primary path table
      ...
      CurrentStreamPointer := FImage.Stream.Position;
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Current Pos After Pri Path Tables: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
     ...
     //write out Supplemental path table
      ...
      CurrentStreamPointer := FImage.Stream.Position;
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Current Pos After Sup Path Tables: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
    ...
    // write Joliet directory and file tables start with FTree.JolietOffsett
      ...
      CurrentStreamPointer := FImage.Stream.Position;
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Current Pos After Write Structure: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
    ...
    //Fill Up Gap to 150 sectors
      ...
      for Index := 1 to PadBytes do FImage.Stream.Write(TempPadChar, 1); // pad out sectors
      Log('SaveImage', '|----------------------------------------------------------|');
    ...
    // load up and write out files data
      ...
      CurrentStreamPointer := FImage.Stream.Position;
      Sector := (FImage.Stream.Position div FImage.SectorDataSize);
      Log('SaveImage', 'Current Pos After Write File Data: ' + inttostr(FImage.Stream.Position) + ' : ' + inttohex(FImage.Stream.Position, 6) + ' : ' + inttostr(Sector));
      Log('SaveImage', '|----------------------------------------------------------|');
...
begin
   ...
   try
      ...
      begin
         if (fImage.SectorDataSize - (Cardinal(lWorkPtr) - Cardinal(lBuffer))) < SizeOf(TRootDirectoryRecord) then
         begin
            lWorkPtr := lBuffer;
            fImage.ReadSector_Data(lWorkPtr^, fImage.SectorDataSize);
         end;
         ...
         New(DR);
         Move(lWorkPtr^, DR^, SizeOf(TRootDirectoryRecord));
         Inc(lWorkPtr, SizeOf(TRootDirectoryRecord));
        ...
        // padding bytes
         if ((SizeOf(TRootDirectoryRecord) + DR.LengthOfFileIdentifier) < DR.LengthOfDirectoryRecord) then
            Inc(lWorkPtr, DR.LengthOfDirectoryRecord - SizeOf(TRootDirectoryRecord) - DR.LengthOfFileIdentifier);
...
begin
   ...
   try
      ...
      repeat
         ...
         Inc(lWorkPtr, 2);
         Move(lWorkPtr^, PathTableEntry.DirectoryIdentifier, PathTableEntry.LengthOfDirectoryIdentifier);
      ...
      until ((Cardinal(lWorkPtr) - Cardinal(lBuffer)) >= (fPVDClass.Descriptor.Primary.PathTableSize.LittleEndian - 2));
   finally
